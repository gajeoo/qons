...
export const getStats = query({
  ...
  handler: async (ctx, args) => {
    const context = await getEffectiveContext(ctx);
    if (!context)
      return { totalHours: 0, activeNow: 0, entries: 0, avgHoursPerDay: 0 };
    ...
    const activeNow = entries.filter(
      (e: any) => e.status === "clocked_in",
    ).length;
    const uniqueDays = new Set(
      filtered.map((e: any) => new Date(e.clockIn).toISOString().split("T")[0]),
    ).size;
    ...
    return {
      ...
      avgHoursPerDay:
        uniqueDays > 0 ? Math.round((totalHours / uniqueDays) * 100) / 100 : 0,
    };
  },
});
...
export const getTimesheetSummary = query({
  ...
  handler: async (ctx, args) => {
    ...
    const staffIds = [
      ...new Set(entries.map((e: any) => e.staffId.toString())),
    ];
  ...
  handler: async (ctx, args) => {
    ...
    if (context.isWorker) {
      ...
      if (args.staffId !== context.linkedStaffId) {
        throw new Error(
          "Workers can only clock in for their own staff profile",
        );
      }
    }
...
export const clockOut = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!entry || entry.userId !== context.ownerUserId)
...
export const approve = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (context.isWorker)
      throw new Error("Workers cannot approve time entries");
