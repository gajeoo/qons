      ...
      if (profile.organizationUserId) {
        const orgSub = await ctx.db
          ...
          .withIndex("by_userId", q =>
            q.eq("userId", profile.organizationUserId!),
          )
          .order("desc")
          .first();
        const orgHasSub =
        ...
        return {
          isOnTrial: false,
          trialExpired: !orgHasSub,
          daysRemaining: orgHasSub ? 999 : 0,
          hasSubscription: orgHasSub,
          plan: orgSub?.plan,
        };
      }
    }
      ...
      users.map(async user => {
        ...
        const teamMembers = await ctx.db
          ...
          .withIndex("by_organizationUserId", q =>
            q.eq("organizationUserId", user._id),
          )
          .take(1000);
...
export const getDashboardStats = query({
  ...
  handler: async ctx => {
    ...
    const activeTrials = profiles.filter(
      p => p.trialEndDate && p.trialEndDate > now && p.role !== "admin",
    ).length;
