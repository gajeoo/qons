...
const PAYPAL_CONFIG = {
  ...
  monthlyPlanId: process.env.PAYPAL_PLAN_MONTHLY || "YOUR_PAYPAL_PLAN_MONTHLY",
  annualPlanId: process.env.PAYPAL_PLAN_ANNUAL || "YOUR_PAYPAL_PLAN_ANNUAL",
  mode: "live" as const,
};
...
async function getPayPalAccessToken(
  ctx: any,
): Promise<{ accessToken: string; mode: string }> {
  ...
  handler: async (ctx, { subscriptionId, billingCycle }) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.subscriptions.upsertFromStripe, {
        userId,
        stripeCustomerId:
          subscriptionData.subscriber?.payer_id || `paypal_customer_${userId}`,
...
export const cancelMySubscription = action({
  ...
  handler: async ctx => {
    ...
    const sub = await ctx.runQuery(
      internal.paypal._getLatestSubscriptionByUser,
      {
        userId,
      },
    );
    ...
    const paypalSubscriptionId = sub.stripeSubscriptionId.replace(
      "paypal_sub_",
      "",
    );
