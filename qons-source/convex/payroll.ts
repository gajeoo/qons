...
export const generate = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const id of staffIds) {
      ...
      if (s)
        staffMap.set(id.toString(), { name: s.name, hourlyRate: s.hourlyRate });
    }

    // Aggregate by staff
    const staffHours = new Map<
      string,
      { regular: number; overtime: number; staffId: (typeof staffIds)[0] }
    >();
    for (const entry of periodEntries) {
      const key = entry.staffId.toString();
      const current = staffHours.get(key) || {
        regular: 0,
        overtime: 0,
    ...
    for (const [staffIdStr, hours] of staffHours) {
      ...
      const regularPay =
        Math.round(hours.regular * staffInfo.hourlyRate * 100) / 100;
      const overtimePay =
        Math.round(hours.overtime * staffInfo.hourlyRate * 1.5 * 100) / 100;
