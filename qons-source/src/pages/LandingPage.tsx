...
function DemoVideo() {
  ...
  const sidebarItems = ["Dashboard", "Properties", "Residents", "Rent", "Maintenance", "Leases", "Accounting", "Analytics"];
  ...
  const screens = [
    {
      ...
      rows: [
        { text: "Rent payment received — Unit 4B", badge: "$1,450", cls: "bg-emerald-500/20 text-emerald-400" },
        { text: "Maintenance — Plumbing issue", badge: "Urgent", cls: "bg-red-500/20 text-red-400" },
        { text: "Lease renewal — Sarah J.", badge: "Pending", cls: "bg-amber-500/20 text-amber-400" },
        { text: "New tenant application", badge: "Review", cls: "bg-sky-500/20 text-sky-400" },
      ],
    },
    {
      ...
      rows: [
        { text: "Unit 4B — John M. paid", badge: "$1,450", cls: "bg-emerald-500/20 text-emerald-400" },
        { text: "Unit 2A — Lisa R. paid", badge: "$1,200", cls: "bg-emerald-500/20 text-emerald-400" },
        { text: "Unit 7C — Mark P. overdue", badge: "3 days", cls: "bg-red-500/20 text-red-400" },
        { text: "Unit 1D — Auto-reminder sent", badge: "Sent", cls: "bg-sky-500/20 text-sky-400" },
      ],
    },
    {
      title: "Maintenance",
      ...
      rows: [
        { text: "Unit 3A — HVAC not working", badge: "Urgent", cls: "bg-red-500/20 text-red-400" },
        { text: "Unit 5B — Leaky faucet", badge: "Assigned", cls: "bg-sky-500/20 text-sky-400" },
        { text: "Unit 2C — Door lock replaced", badge: "Done", cls: "bg-emerald-500/20 text-emerald-400" },
        { text: "Unit 8A — Paint touch-up", badge: "Scheduled", cls: "bg-amber-500/20 text-amber-400" },
      ],
    },
    {
      title: "Analytics",
      ...
      rows: [
        { text: "Monthly P&L report ready", badge: "Download", cls: "bg-sky-500/20 text-sky-400" },
        { text: "Revenue up 12% vs last month", badge: "+12%", cls: "bg-emerald-500/20 text-emerald-400" },
        { text: "Maintenance costs down 8%", badge: "-8%", cls: "bg-emerald-500/20 text-emerald-400" },
        { text: "3 leases expiring this month", badge: "Action", cls: "bg-amber-500/20 text-amber-400" },
      ],
    },
  ];

  useEffect(() => {
    const timer = setInterval(() => setActiveScreen((s) => (s + 1) % screens.length), 4000);
    return () => clearInterval(timer);
  }, [screens.length]);

  const screen = screens[activeScreen];
  const sidebarIdx = sidebarItems.findIndex((s) => s.toLowerCase() === screen.title.toLowerCase()) ?? 0;
    ...
    <div className="max-w-4xl mx-auto">
      <div className="relative aspect-video rounded-2xl border-2 border-slate-200 dark:border-slate-800 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 overflow-hidden shadow-2xl shadow-slate-200/50 dark:shadow-slate-900/50">
        <div className="absolute inset-0 p-3 md:p-6 flex flex-col">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 h-5 rounded-full bg-white/10 max-w-xs mx-auto flex items-center px-3">
              <span className="text-[9px] text-white/40 font-mono transition-all duration-500">{screen.url}</span>
            </div>
          </div>
            ...
            <div className="flex-1 flex flex-col gap-2 overflow-hidden">
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {screen.stats.map((s, i) => (
                  ...
                  >
                    <p className="text-white/40 text-[8px] md:text-[9px]">{s.label}</p>
                    <p className={`font-bold text-xs md:text-base ${s.color}`}>{s.value}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="flex-1 rounded-lg bg-white/5 p-2.5 overflow-hidden">
                <p className="text-white/40 text-[9px] font-semibold mb-1.5 uppercase tracking-wider">
                  {screen.title === "Dashboard" ? "Recent Activity" : screen.title}
                </p>
                <div className="space-y-1.5">
                  {screen.rows.map((row, i) => (
                    <div
                      ...
                      style={{ animation: `fadeSlideUp 0.4s ease-out ${0.15 + i * 0.1}s both` }}
                    >
                      <span className="text-white/60 text-[9px] md:text-[11px] truncate mr-2">{row.text}</span>
                      <span className={`text-[8px] md:text-[9px] px-1.5 py-0.5 rounded-full ${row.cls} shrink-0 font-medium`}>{row.badge}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="absolute top-2 right-2 md:top-3 md:right-3 flex items-center gap-1.5 z-10 pointer-events-none">
          ...
          <span className="text-[10px] font-semibold text-white/70 hidden md:block">{APP_NAME}</span>
        </div>
      ...
      <section className="py-20 sm:py-28 bg-white dark:bg-slate-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-16">
            ...
            <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">
              One platform replaces your entire toolkit — from rent collection to
