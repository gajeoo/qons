...
type PropertyForm = {
  ...
  state: string;
  zip: string;
  type: "residential" | "commercial" | "mixed" | "hoa";
  units: string;
  contactName: string;
  contactPhone: string;
  contactEmail: string;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage all your buildings and properties
          </p>
        </div>
        <Button
          onClick={openCreate}
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Upload a CSV or JSON file. QuonsApp will auto-categorize rows into
              properties, staff, residents, and property image updates.
            </p>
            <Input
              type="file"
              accept=".csv,.json"
            ...
            {importSummary ? (
              <p className="text-sm rounded-md border p-3 bg-muted">
                {importSummary}
              </p>
            ) : null}
            <p className="text-xs text-muted-foreground">
              Tip: include columns like propertyName, address, city, state, zip,
              units, imageUrl, role, hourlyRate, residentName, unit, and
              leaseEnd for best results.
            </p>
              ...
              <div className="space-y-2 pt-2">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Select
                    value={historyFilter}
                    onValueChange={(
                      value: "all" | "success" | "partial" | "failed",
                  ...
                  {filteredImportHistory.length === 0 ? (
                    ...
                    filteredImportHistory.map(job => (
                      ...
                      >
                        ...
                        <p className="text-muted-foreground mt-1">
                          Rows: {job.totalRows} | Properties +
                          {job.propertiesCreated}/{job.propertiesUpdated} |
                          Staff +{job.staffCreated} | Residents +
                          {job.residentsCreated}
                        </p>
                        {job.ignoredRows > 0 || job.errorCount > 0 ? (
                          <p className="text-muted-foreground mt-1">
                            Ignored: {job.ignoredRows} | Errors:{" "}
                            {job.errorCount}
                          </p>
                        ) : null}
                        {job.errorSummary ? (
