...
export function SchedulePage() {
  ...
  const getStaffName = (id?: Id<"staff">) =>
    id ? staffList.find(s => s._id === id)?.name || "Unknown" : null;
    ...
    <FeatureGate feature="schedule">
      <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {isWorker
                ...
                : "Schedule and manage shifts across all properties"}
            </p>
          </div>
        ...
        <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
          {[
            {
              label: isWorker ? "My Shifts" : "Total Shifts",
              value: stats?.total ?? 0,
              icon: Calendar,
            },
            { label: "Scheduled", value: stats?.scheduled ?? 0, icon: Clock },
            {
              label: isWorker ? "In Progress" : "Open (Unfilled)",
              value: isWorker
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium ml-2">
              {viewMode === "week"
                ...
                : new Date(`${currentDate}T12:00:00`).toLocaleDateString(
                    "en-US",
                    {
                      weekday: "long",
                      month: "long",
          ...
          <div className="grid grid-cols-7 gap-2">
            {weekDays.map(day => {
              ...
              return (
                <div
                  key={day.date}
                  className={`min-h-[200px] border rounded-lg p-2 ${day.isToday ? "border-teal bg-teal/5" : ""}`}
                >
                  <p
                    className={`text-xs font-medium mb-2 ${day.isToday ? "text-teal" : "text-muted-foreground"}`}
                  >
                    {day.label}
                  </p>
                  <div className="space-y-1.5">
                    {dayShifts.map(s => (
                      <div
                        key={s._id}
                        className={`text-[11px] p-1.5 rounded ${isWorker ? "" : "cursor-pointer hover:opacity-80"} ${s.aiAssigned ? "border-l-2 border-teal" : ""} ${statusColor[s.status]?.replace("text-", "").includes("amber") ? "bg-amber-50 border border-amber-200" : "bg-blue-50 border border-blue-200"}`}
                      >
                        {isWorker ? (
                          /* Workers: read-only view */
                          <div>
                            <p className="font-medium truncate">
                              {getPropertyName(s.propertyId)}
                            </p>
                            <p className="text-[10px] opacity-70">
                              {s.startTime} - {s.endTime}
                            </p>
                            <Badge
                              className={`text-[9px] mt-0.5 ${statusColor[s.status]}`}
                            >
                              {s.status.replace("_", " ")}
                            </Badge>
                          </div>
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger className="w-full text-left">
                              ...
                              {s.aiAssigned && (
                                <Bot className="size-2.5 inline text-teal" />
                              )}
                            </DropdownMenuTrigger>
                            <DropdownMenuContent>
                              {s.status === "open" && (
                                <DropdownMenuItem
                                  onClick={() => handleFindCoverage(s._id)}
                                >
                                  <Zap className="size-4" /> Find Coverage
                                </DropdownMenuItem>
                              )}
                              <DropdownMenuItem
                                onClick={() =>
            ...
            {shifts.length === 0 ? (
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-muted-foreground">
                    {isWorker
                      ? "No shifts assigned to you for this day"
                      ...
                      <div>
                        ...
                        {!isWorker && (
                          <p className="text-sm text-muted-foreground">
                            {getStaffName(s.staffId) || "Unassigned"}
                          </p>
                        )}
                      </div>
            ...
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Leave open for AI" />
                    </SelectTrigger>
                    <SelectContent>
