...
function PayrollPageInner() {
  ...
  const [form, setForm] = useState({
    periodStart: "",
    periodEnd: "",
    format: "csv" as "csv" | "adp" | "paychex" | "quickbooks" | "excel",
  });
  ...
  const exportCsv = (payroll: (typeof payrolls)[0]) => {
    const header =
      "Staff,Regular Hours,OT Hours,Regular Pay,OT Pay,Total Pay\n";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Generate and export payroll reports with overtime calculations
          </p>
        </div>
        <Button
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-3">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div className="rounded-lg bg-teal/10 p-2">
                <BadgeDollarSign className="size-4 text-teal" />
              </div>
              <div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div className="rounded-lg bg-amber-100 p-2">
                <FileSpreadsheet className="size-4 text-amber-700" />
              </div>
              <div>
                <p className="text-2xl font-bold">
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground mt-1">
              Generate a payroll report from approved time entries
            </p>
            <Button
              onClick={() => setShowForm(true)}
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {p.staffCount} staff · {p.totalHours}h ·{" "}
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-right mr-3">
                      <p className="font-bold text-lg">
                        ${(p.totalWithTax ?? p.totalAmount).toLocaleString()}
                      </p>
                      {(p.taxAmount ?? 0) > 0 ? (
                        <p className="text-xs text-muted-foreground">
                          incl. tax ${p.taxAmount?.toLocaleString()} (
                          {p.taxRate ?? 0}%)
                        </p>
                      ) : null}
                    </div>
