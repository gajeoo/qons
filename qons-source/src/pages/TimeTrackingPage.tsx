...
function TimeTrackingPageInner() {
  ...
  const periodEndTs = periodEnd
    ? new Date(`${periodEnd}T23:59:59`).getTime()
    : undefined;
  ...
  const timesheet = useQuery(api.timeTracking.getTimesheetSummary, {
  ...
  const handleClockIn = async () => {
    ...
    if (!staffId || !propertyId) {
      toast.error(isWorker ? "Select a property" : "Select staff and property");
      return;
    }
    setLoading("clock-in");
    try {
  ...
  const getStaffName = (id: Id<"staff">) =>
    staffList.find(s => s._id === id)?.name || "Unknown";
  const getPropertyName = (id: Id<"properties">) =>
    properties.find(p => p._id === id)?.name || "Unknown";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            {isWorker
              ? "Clock in/out and view your time records"
              : "GPS-verified clock-in and time tracking"}
          </p>
        </div>
        <Button
          ...
          <CardContent className="p-4 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Includes all owner + sub-account clock-outs using each staff
                  member&apos;s hourly rate.
                </p>
              </div>
              <Badge className="bg-teal/10 text-teal">
                {timesheet?.staffCount ?? 0} staff
              </Badge>
            </div>
              ...
              <div className="md:col-span-2 lg:col-span-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {taxSettings?.canEdit && !isSubAccount ? (
                    <Button variant="outline" onClick={handleSaveTaxRate}>
                      Save
                    </Button>
                  ) : (
            ...
            <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">
                  ${(timesheet?.totalAmount ?? 0).toLocaleString()}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">
                  Tax ({timesheet?.taxRate ?? 0}%)
                </p>
                <p className="text-lg font-semibold">
                  ${(timesheet?.taxAmount ?? 0).toLocaleString()}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Total w/ Tax</p>
                <p className="text-lg font-semibold">
                  ${(timesheet?.totalWithTax ?? 0).toLocaleString()}
                </p>
              </div>
            </div>
            ...
            {timesheet?.staff?.length ? (
              ...
              <p className="text-sm text-muted-foreground">
                No completed/approved entries yet for payroll calculation.
              </p>
            )}
          </CardContent>
        </Card>
                ...
                <CardContent className="p-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-green-700 font-medium mt-1">
                      ...
                      {formatDuration(Date.now() - entry.clockIn)} elapsed
                    </p>
                    {entry.clockInLat && (
        ...
        {timeEntries.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-muted-foreground">
                {isWorker
                  ? "No time entries recorded yet. Clock in to start tracking."
                  : "No time entries yet. Clock in a staff member to start tracking."}
              </p>
            </CardContent>
          </Card>
        ) : (
          ...
          <div className="space-y-4">
            ...
            {isWorker ? (
              ...
              <div>
                <Label>Staff Member *</Label>
                <Select
                  value={clockInForm.staffId}
                  onValueChange={v =>
                    setClockInForm({ ...clockInForm, staffId: v })
                  }
                >
                  <SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={v =>
                  setClockInForm({ ...clockInForm, propertyId: v })
                }
              >
                <SelectTrigger>
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1">
              <MapPin className="size-3" /> GPS location will be captured
              automatically
            </p>
            <div className="flex gap-3 justify-end">
              <Button variant="outline" onClick={() => setShowClockIn(false)}>
                Cancel
              </Button>
              <Button
