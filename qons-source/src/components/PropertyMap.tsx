...
export function PropertyMap({
  ...
  useEffect(() => {
    ...
    const loadMap = async () => {
      try {
        ...
        if (mappableProperties.length === 1) {
          center = [
            mappableProperties[0].latitude!,
            mappableProperties[0].longitude!,
          ];
          zoom = 15;
    ...
    // Re-run when the list of mappable properties changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
    JSON.stringify(mappableProperties.map(p => `${p._id}:${p.latitude}:${p.longitude}`)),
    showControls,
  ]);
          ...
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center">
            <div className="text-center p-6">
              ...
              <h3 className="font-semibold text-lg mb-1">
                No properties on map yet
              </h3>
              <p className="text-sm text-muted-foreground max-w-sm">
                Add latitude and longitude to your properties to see them on the
                interactive map.
              </p>
            </div>
          </div>
        ...
        <Card className="border-amber-200 bg-amber-50/50">
          <CardContent className="p-4">
            <p className="text-sm font-medium text-amber-800 mb-2">
              {unmappedProperties.length} propert
              {unmappedProperties.length === 1 ? "y" : "ies"} without
              coordinates
            </p>
            <div className="flex flex-wrap gap-2">
              ...
              {unmappedProperties.length > 5 && (
                <span className="text-xs text-amber-600">
                  +{unmappedProperties.length - 5} more
                </span>
              )}
            </div>
