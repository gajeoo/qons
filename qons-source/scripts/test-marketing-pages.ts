...
runTest("Marketing Pages Navigation", async helper => {
  ...
  if (!heroHeading?.includes("Manual Scheduling")) {
    throw new Error(
      `Expected hero to mention 'Manual Scheduling', got: ${heroHeading}`,
    );
  }
  ...
  const featureSections = await page
    .locator(
      "#ai-scheduling, #multi-property, #analytics, #mobile, #payroll, #amenity-booking, #hoa-management",
    )
    .count();
  ...
  const proVisible = await page
    .locator("text=Professional")
    .first()
    .isVisible();
  const enterpriseVisible = await page
    .locator("text=Enterprise")
    .first()
    .isVisible();
  ...
  const founderVisible = await page
    .locator("text=Ernest Owusu")
    .first()
    .isVisible();
  if (!founderVisible) {
