...
export const sendEmail = action({
  ...
  handler: async (ctx, args) => {
    const config = await ctx.runQuery(internal.siteConfig.getInternal, {});
    const apiKey = process.env.SENDGRID_API_KEY || config?.sendgridApiKey;
    if (!apiKey) {
      console.log(
        "[Email] SendGrid not configured, logging email:",
        args.to,
        args.subject,
      );
      return {
...
export const sendBatchEmail = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!apiKey) {
      console.log(
        "[Email] SendGrid not configured, logging batch to",
...
export const sendSms = action({
  ...
  handler: async (_ctx, args) => {
    ...
    if (!accountSid || !authToken || !fromNumber) {
      console.log(
        "[SMS] Twilio not configured, logging SMS:",
        args.to,
        args.body.substring(0, 50),
      );
...
export const sendBatchSms = action({
  ...
  handler: async (_ctx, args) => {
    ...
    if (!accountSid || !authToken || !fromNumber) {
      console.log(
        "[SMS] Twilio not configured, logging batch SMS to",
        args.recipients.length,
        "recipients",
      );
...
export const getStatus = action({
  ...
  handler: async ctx => {
    const config = await ctx.runQuery(internal.siteConfig.getInternal, {});
    return {
      emailConfigured: !!(process.env.SENDGRID_API_KEY || config?.sendgridApiKey),
      smsConfigured: !!(
