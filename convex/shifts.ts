...
export const listByDate = query({
  ...
  handler: async (ctx, args) => {
    ...
    const shifts = await ctx.db
      .query("shifts")
      .withIndex("by_userId_date", (q: any) => q.eq("userId", ownerUserId).eq("date", args.date))
      .collect();
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!shift || shift.userId !== context.ownerUserId) throw new Error("Not found");
...
export const aiSchedule = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const shifts = await ctx.db
      .query("shifts")
      .withIndex("by_userId_date", (q: any) => q.eq("userId", ownerUserId).eq("date", targetDate))
      .collect();
    ...
    const busyStaffIds = new Set(
      scheduledShifts.filter((s: any) => s.staffId).map((s: any) => s.staffId!.toString())
    );
...
export const findCoverage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const dayShifts = await ctx.db
      .query("shifts")
      .withIndex("by_userId_date", (q: any) => q.eq("userId", ownerUserId).eq("date", shift.date))
      .collect();
    const busyStaffIds = new Set(
      dayShifts.filter((s: any) => s._id !== args.shiftId && s.staffId && s.status !== "cancelled")
    ...
    const available = allStaff
      .filter((s: any) => s.status === "active" && !busyStaffIds.has(s._id.toString()))
      .map((staff: any) => {
        let score = 50;
        const dayAvail = staff.availability?.[dayOfWeek as keyof typeof staff.availability];
