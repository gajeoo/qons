...
export const listConversations = query({
  ...
  returns: v.array(
    v.object({
      ...
      visitorName: v.optional(v.string()),
      visitorEmail: v.optional(v.string()),
      status: v.union(v.literal("active"), v.literal("closed"), v.literal("waiting_admin")),
...
export const getStats = query({
  ...
  handler: async (ctx) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) return { totalConversations: 0, activeConversations: 0, waitingForAdmin: 0, totalUnread: 0 };
    ...
    if (!profile || profile.role !== "admin")
      return { totalConversations: 0, activeConversations: 0, waitingForAdmin: 0, totalUnread: 0 };
...
function generateAiResponse(userMessage: string): string {
  ...
  if (msg.includes("feature") || msg.includes("what can") || msg.includes("what does") || msg.includes("do you")) {
  ...
  if (msg.includes("hoa") || msg.includes("association") || msg.includes("violation") || msg.includes("dues")) {
  ...
  if (msg.includes("human") || msg.includes("agent") || msg.includes("real person") || msg.includes("speak to")) {
  ...
  if (msg.includes("property") || msg.includes("building") || msg.includes("portfolio")) {
  ...
  if (msg.includes("team") || msg.includes("invite") || msg.includes("member") || msg.includes("sub-account") || msg.includes("worker")) {
