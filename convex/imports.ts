        ...
        if (Array.isArray(parsed)) {
          ...
          for (const key of ["properties", "staff", "residents"]) {
            if (Array.isArray((parsed as Record<string, unknown>)[key])) {
              grouped.push(
                ...((parsed as Record<string, unknown>)[key] as GenericRecord[]),
              );
            }
          }
          records = grouped;
    ...
    for (const record of records) {
      try {
        ...
        if (rowType === "property") {
          ...
          const address = getValue(record, ["address", "street", "propertyAddress"]);
          const city = getValue(record, ["city"]);
          const state = getValue(record, ["state"]);
          const zip = getValue(record, ["zip", "zipcode", "postalCode"]);
          const units =
            Number.parseInt(getValue(record, ["units", "totalUnits"]) || "0", 10) || 0;
        ...
        if (rowType === "staff") {
          ...
          const role = getValue(record, ["role", "position", "jobTitle"]) || "staff";
          const hourlyRate =
            Number.parseFloat(
        ...
        if (rowType === "resident") {
          ...
          const propertyName = getValue(record, ["property", "propertyName", "building"]);
          const phone = getValue(record, ["phone", "mobile"]);
          const leaseEnd = getValue(record, ["leaseEnd", "lease_end"]);

          const linkedProperty = propertyByName.get(propertyName.trim().toLowerCase());
