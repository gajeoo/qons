...
export function RoleSelector({
  ...
  const showCustomOption = search && !PRESET_ROLES.some(
        ...
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white dark:bg-slate-900 border rounded-lg shadow-lg">
          ...
          <div className="max-h-48 overflow-y-auto">
            ...
            {filtered.map((role) => (
              ...
              >
                {value === role.toLowerCase() && <Check className="size-3.5 text-teal" />}
                <span className={value === role.toLowerCase() ? "font-medium" : ""}>{role}</span>
              </button>
            ))}
            {filtered.length === 0 && !showCustomOption && (
              <p className="px-3 py-4 text-sm text-muted-foreground text-center">No roles found</p>
            )}
          </div>
        </div>
      )}
