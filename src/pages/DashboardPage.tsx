  ...
  return (
    <div className="space-y-8">
            ...
            <div className="space-y-3">
              {setupSteps.map((step, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  {!step.done && step.label === "Choose a plan" && !isSubAccount && (
                    <Button variant="outline" size="sm" className="ml-auto" asChild>
                      <Link to="/pricing">Choose Plan <ArrowRight className="size-3" /></Link>
                    </Button>
                  )}
                  {!step.done && step.label === "Complete onboarding" && (hasBillingAccess || isSubAccount) && (
                    <Button variant="outline" size="sm" className="ml-auto" asChild>
                      <Link to="/onboarding">Complete Setup <ArrowRight className="size-3" /></Link>
                    </Button>
                  )}
                </div>
              ))}
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        {(isWorker
          ? [
              { label: "My Shifts", value: shiftStats?.scheduled ?? 0, sub: `${shiftStats?.total ?? 0} total`, icon: Calendar, color: "text-purple-600 bg-purple-100" },
                ...
                {hasBillingAccess ? (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`px-3 py-1 rounded-full text-xs font-medium ${subscription?.status === "active" || isPaidSubscriber ? "bg-teal/10 text-teal" : subscription?.status === "past_due" ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}>
                        {subscription
        ...
        <div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Link to="/schedule" className="group">
              <Card className="h-full hover:shadow-md transition-all hover:border-teal/30">
                <CardContent className="p-5 flex items-center gap-4">
                  <div className="rounded-xl p-3 text-purple-600 bg-purple-100"><Calendar className="size-6" /></div>
                  <div>
                    <h3 className="font-semibold text-sm">My Shifts</h3>
                    <p className="text-xs text-muted-foreground mt-0.5">View your assigned shifts</p>
                  </div>
                </CardContent>
              </Card>
            </Link>
            <Link to="/time-tracking" className="group">
              <Card className="h-full hover:shadow-md transition-all hover:border-teal/30">
                <CardContent className="p-5 flex items-center gap-4">
                  <div className="rounded-xl p-3 text-amber-600 bg-amber-100"><Clock className="size-6" /></div>
                  <div>
                    <h3 className="font-semibold text-sm">Time Tracking</h3>
                    <p className="text-xs text-muted-foreground mt-0.5">View your clock-in records</p>
                  </div>
                </CardContent>
              </Card>
            </Link>
