...
function formatMessage(text: string): string {
  return text
    ...
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="underline text-sky-500">$1</a>')
    .replace(/\n/g, "<br />");
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">View and reply to customer conversations</p>
      </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{chatStats?.totalConversations ?? 0}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{chatStats?.activeConversations ?? 0}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{chatStats?.waitingForAdmin ?? 0}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{chatStats?.totalUnread ?? 0}</p>
          ...
          <div className="flex-1 overflow-y-auto">
            {conversations.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground text-sm p-4">
                ...
                <p className="text-xs">Chats from the widget will appear here</p>
              </div>
            ) : (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge
                          variant={conv.status === "active" ? "default" : conv.status === "waiting_admin" ? "destructive" : "secondary"}
                          className="text-[9px] h-4 px-1.5"
                        >
                          {conv.status === "waiting_admin" ? "Needs Reply" : conv.status}
                        </Badge>
        ...
        <Card className="overflow-hidden flex flex-col">
          {selectedId && selectedConversation ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {selectedConversation.visitorEmail && (
                      <p className="text-xs text-muted-foreground">{selectedConversation.visitorEmail}</p>
                    )}
                  </div>
                </div>
                  ...
                  >
                    <div className="flex flex-col max-w-[80%]">
                      <div className="flex items-center gap-1.5 mb-1 text-[10px] text-muted-foreground">
                        {msg.role === "visitor" ? <User className="size-3" /> : msg.role === "admin" ? <Shield className="size-3" /> : <Bot className="size-3" />}
                        <span>{msg.role === "visitor" ? "Visitor" : msg.role === "admin" ? (msg.senderName || "Admin") : "AI"}</span>
              ...
              {selectedConversation.status === "closed" && (
                <div className="p-3 border-t bg-muted/50 text-center">
                  <p className="text-sm text-muted-foreground flex items-center justify-center gap-1.5">
                    <CheckCircle className="size-4" /> This conversation is closed
                  </p>
                </div>
              )}
            </>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground">
              ...
              <p className="text-xs mt-1">Choose from the list to view messages and reply</p>
            </div>
          )}
        </Card>
      </div>
