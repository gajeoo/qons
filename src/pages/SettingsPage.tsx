...
import { ChevronRight, CreditCard, Loader2, Moon, Palette, Sun, User } from "lucide-react";
...
export function SettingsPage() {
  ...
  const payrollTaxSettings = useQuery(api.payroll.getTaxSettings);
  const { isSubAccount, isPaidSubscriber } = useFeatureAccess();
  const { signIn, signOut } = useAuthActions();
  const deleteAccount = useMutation(api.users.deleteAccount);
  const setTaxSettings = useMutation(api.payroll.setTaxSettings);
  const cancelMySubscription = useAction(api.paypal.cancelMySubscription);
  ...
  useEffect(() => {
    if (payrollTaxSettings) {
      setTaxRateInput(String(payrollTaxSettings.taxRate));
    }
  }, [payrollTaxSettings]);
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        setSuccess("Membership cancellation scheduled for the end of your billing cycle.");
      }
    } catch {
  ...
  const handleSaveTaxRate = async () => {
    ...
    setSuccess("");
    try {
      const result = await setTaxSettings({ taxRate: parsed });
      setTaxRateInput(String(result.taxRate));
      setSuccess("Payroll tax updated successfully.");
    } catch (e: any) {
      setError(e?.message || "Unable to update payroll tax");
    } finally {
      setTaxLoading(false);
    }
  };
    ...
    <div className="space-y-8 max-w-2xl mx-auto">
            ...
            <div className="rounded-lg border p-4">
              ...
              {subscription?.cancelAtPeriodEnd ? (
                <p className="text-xs text-amber-600 mt-1">
                  Membership cancellation is scheduled at period end.
                </p>
              ) : null}
            </div>

            {isPaidSubscriber && !subscription?.cancelAtPeriodEnd && (
              <Button
                variant="outline"
                className="w-full"
      ...
      {!isSubAccount && (
        <Card>
          ...
          <CardContent className="space-y-3">
            <div>
              <Label htmlFor="payrollTaxRate">Tax Rate (%)</Label>
              <div className="mt-2 flex items-center gap-2">
                <Input
                  ...
                  disabled={!payrollTaxSettings?.canEdit}
                />
                <Button
                  variant="outline"
                  onClick={handleSaveTaxRate}
                  disabled={taxLoading || !payrollTaxSettings?.canEdit}
                >
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Applied automatically in payroll and timesheet totals.
              </p>
            </div>
            {!payrollTaxSettings?.canEdit ? (
              <p className="text-xs text-muted-foreground">
                Only the primary account can update payroll tax settings.
              </p>
            ) : null}
          </CardContent>
        </Card>
