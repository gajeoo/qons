...
- **npm** — Default package manager for local development and deployment
...
npm run test -- scripts/demo-test.ts

# Or run multiple tests
npm run test -- scripts/test-feature1.ts scripts/test-feature2.ts
```
...
npm run screenshot -- /settings settings.png
```
...
| `npm run test -- <file>`           | Run e2e test (starts server, runs test, stops) |
...
| `npm run screenshot -- [path] [name]` | Take screenshot of a page                   |
...
This app can be deployed to Vercel as a standard Vite SPA.

1. Import the repository into Vercel.
2. Keep the default build command as `npm run build`.
3. Keep the output directory as `dist`.
4. Add `VITE_CONVEX_URL` in the Vercel project environment variables.
5. Redeploy after saving the environment variable.

If `VITE_CONVEX_URL` is missing, the deployed app will render a configuration
screen instead of failing with a blank page.

For a step-by-step checklist and exact environment matrix, see
`VERCEL_DEPLOYMENT.md` and `.env.vercel.example`.
